# 👋 Hi, I’m a SProduct Manager in Healthcare Technology

I’m a **Product Manager** specializing in **healthcare technology**, with a track record of building products that improve clinical workflows, patient access, **interoperability, regulatory complexity, user centered and design** are areas I've sharpend my skills in.

I’m especially motivated by products that reduce patient friction in healthcare whether that’s eligibility, benefits, prescribing, or general data exchange.

---

## 🔍 Current Focus

- 🏥 **Interoperability & Standards**  
  Building products aligned with HL7® FHIR®, NCPDP, and real-world payer/provider workflows.

- 📈 **Outcome-Driven Product Strategy**  
  Connecting roadmap decisions to adoption, revenue, and customer impact.

- 🤝 **Cross-Functional Leadership**  
  Partnering closely with dev engineering, design, clinical, legal, and marketing teams.

- ⚖️ **Healthcare Compliance & Trust**  
  Designing for HIPAA compliant architectures, and enterprise reliability.

---

## 🧠 Skills & Expertise

| Category | Skills |
|--------|-------|
| **Product Management** | Product strategy, roadmap ownership, discovery & delivery, PRDs, OKRs, portfolio prioritization |
| **Healthcare Domain** | EHR integrations, benefits & eligibility, prescribing workflows, data exchange |
| **Technical** | APIs, platform products, system integrations, data pipelines, SDLC |
| **AI** | prompt engineering, agentic workflows |
| **User Centered Design** | Customer discovery, usability testing, journey mapping |
| **Analytics & Growth** | KPI definition, experimentation, adoption metrics, revenue enablement |
| **Leadership** | Stakeholder alignment, executive communication, mentoring PMs |

---

## 🛠️ Tools I Work With

- **Product:** Jira, Confluence, Aha!, Productboard
- **Design & Research:** Figma, Claude, Copilot
- **Data:** SQL, Looker, Tableau, BigQuery
- **Collaboration:** GitHub, Teams, Miro

---

## 📫 Let’s Connect

I’m always interested in conversations about:
- Healthcare Tech strategy  
- Scaling B2B SaaS in regulated industries  
- Product leadership and mentorship  

👉 **LinkedIn:** [_add link_ ](https://www.linkedin.com/in/nchambers918/) 
👉 **Email:** nchambers918@gmail.com

---

“Building solutions that will change the way patients and consumers interact in healthcare...for the better”
